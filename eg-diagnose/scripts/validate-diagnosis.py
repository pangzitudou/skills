#!/usr/bin/env python3
"""Validate EG diagnosis artifacts before declaring diagnosis complete."""
...
def validate_query_plan(path: Path, run_id: str, errors: list[str]) -> None:
    ...
    for index, query in enumerate(queries):
        ...
        if operation not in {"SELECT", "SHOW", "DESCRIBE", "EXPLAIN", "ES_SEARCH", "LOG_SEARCH", "HTTP_GET", "LOCAL_READ"}:
...
def validate(data: dict[str, Any], diagnosis_path: Path) -> list[str]:
    errors: list[str] = []
    ...
    if data.get("stage") != "diagnosis-complete":
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate EG diagnosis.yml.")
    parser.add_argument("path", help="/tmp/eg/<run-id>/diagnosis.yml")
    args = parser.parse_args()

    path = Path(args.path)
    data = load_yaml(path)
    if not isinstance(data, dict):
        print("ERROR: diagnosis.yml must be an object", file=sys.stderr)
        return 2
